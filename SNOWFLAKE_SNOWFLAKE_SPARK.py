...
def SNOWFLAKE_SNOWFLAKE_PYSPARK():
    ...
    conf.set("spark.sql.debug.maxToStringFields", 10000000)
    ...
    with placeholder.container():
        st.markdown("<h1 style = 'text-align:center; color: Black;font-size:40px;'>SNOWFLAKE TO SNOWFLAKE DATA COMPARE</h1>", unsafe_allow_html = True)
        st.markdown('<hr style = "border-top: 3px solid orange;">',unsafe_allow_html = True)
        st.markdown (""" **_SPARK framework supports comparison of datasets in SNOWFLAKE DWH across various test regions (INTEG and ACCP)._** <br>
                    <br>
                """,True)
        blink_style = """
        @keyframes blink { 
    ...
    try:
        ...
        with st.sidebar.form(key='MyForm',clear_on_submit=True):
            st.file_uploader("Upload parameter file", type=["xlsx"],key = "S2S_Spark_DB")
            pwd1=st.text_input("Please Enter Source database password", type="password",key = "S2Ssourcepassword")
            pwd2=st.text_input("Please Enter Target database password", type="password",key = "S2Stargetpassword")

            
            def DB_CONNECT():
                global SFURL,SFAccount,SFUser,SFDatabase,SFSchema,SFWarehouse,SFRole,SFURL2,SFAccount2,SFUser2,SFDatabase2,SFSchema2,SFWarehouse2,SFRole2
                if st.session_state.S2S_Spark_DB:
                # Parameterising the db connection details#
                    df_db = pd.read_excel(st.session_state.S2S_Spark_DB, sheet_name='Connection')
                    ...
                    SFAccount2 = str(df_db.at[23, "Value"])
                    SFUser2 = str(df_db.at[24, "Value"])
                    SFDatabase2 = str(df_db.at[25, "Value"])
                    SFSchema2 = str(df_db.at[26, "Value"])
                    SFWarehouse2 = str(df_db.at[27, "Value"])
                    SFRole2 = str(df_db.at[28, "Value"])
                    SFURL2 = SFAccount2 +'.snowflakecomputing.com'
                    # Output = str(df_db.at[24, "Value"])
            
                    if 'conn1' not in st.session_state:
                        st.session_state.conn1  = connect_to_snowflake(SFAccount,SFUser,st.session_state.S2Ssourcepassword,SFRole,SFWarehouse,SFDatabase);
                    if 'conn2' not in st.session_state:
                        st.session_state.conn2  = connect_to_snowflake(SFAccount2,SFUser2,st.session_state.S2Stargetpassword,SFRole2,SFWarehouse2,SFDatabase2);
        ...
        def COMPARE():
            placeholder.empty()
            df_dbq = pd.read_excel(st.session_state.S2S_Spark_DB, sheet_name='Snowflake_Snowflake')
            #st.write(pwd)
            i = 2
            ...
            SNOWFLAKE_SOURCE_NAME2 = "net.snowflake.spark.snowflake"
            SNOWFLAKE_OPTIONS = {
			'sfURL': os.environ.get("SNOWFLAKE_URL", SFURL2),
			'sfAccount': os.environ.get("SNOWFLAKE_ACCOUNT", SFAccount2),
			'sfUser': os.environ.get("SNOWFLAKE_USER", SFUser2),
			'sfPassword': os.environ.get("SNOWFLAKE_PASSWORD", pwd2),
			'sfDatabase': os.environ.get("SNOWFLAKE_DATABASE", SFDatabase2),
			'sfSchema': os.environ.get("SNOWFLAKE_SCHEMA", SFSchema2),
			'sfWarehouse': os.environ.get("SNOWFLAKE_WAREHOUSE", SFWarehouse2),
			'sfRole':  os.environ.get("SNOWFLAKE_ROLE", SFRole2)
			}
            
            df2 = spark.read.format(SNOWFLAKE_SOURCE_NAME2) \
            .options(**SNOWFLAKE_OPTIONS) \
            .option('query', TGTQuery) \
            .option("fetchSize", 10000).option("numPartitions", 20) \
            .load()
