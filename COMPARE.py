...
def COMPARE_DB(dataframe1,dataframe2,PKEY_LIST,start_time):
    st.header('**Data Differences**')
    Key_List(dataframe1,dataframe2)
    
    # st.write(start_time)
    letter_list = PKEY_LIST.split(",")  # Spliting values in case user gives Multiple columns in Primary Column
   
    compare = datacompy.Compare( dataframe1, dataframe2, join_columns=letter_list, df1_name='Source', df2_name='Target')
    buffer2 = io.StringIO() # Create a file-like buffer to receive PDF data.
    buffer2.write(compare.report())  # Write report content to the buffer
    value2 = buffer2.getvalue()  # Get the value of the buffer
    archive = io.BytesIO()
    buffer=io.BytesIO()
    is_match = compare.matches(ignore_extra_columns=True)
    if is_match:
        st.success("**_✅ Exact match between source and target data.!! Please download the compare report for further reference._**")
        Download_Txt_File(compare.report(sample_count=100))
        # with zipfile.ZipFile(archive, "w") as zf:
            # zf.writestr("Data_Compare_Report.txt", value2)
    else:
        st.markdown("_Below are the sample data mismatches. This section will be empty if there are no mismatches in the datasets. Please refer to compare report for further reference._")
        st.error("**_❌ Source and Target data does not match._**")
        ...
        with zipfile.ZipFile(archive, "w") as zf:
            ...
            with zf.open("Mismatch.xlsx", "w") as buffer:
                with pd.ExcelWriter(buffer) as writer:
                    for col in final_list:
                        # st.write(col[0:20])
                        df3=compare.sample_mismatch(col.lower(), sample_count=10, for_display=True)
                        df4=compare.sample_mismatch(col.lower(), sample_count=100, for_display=True)
                        if not df3.empty:
...
def COMPARE_MQDB(Connection,df_dbq):
    ...
    for index,row in df_dbq.iterrows():
        ...
        PrimaryKey=row["Target_Primarykey"]
        ...
        compare = datacompy.Compare( df1, df2, join_columns=letter_list, df1_name='Source', df2_name='Target')
        final_list = [e for e in compare.intersect_columns() if e not in newList]
...
def COMPARE_MQDB_DB(Connection1,Connection2,df_dbq):
    st.markdown("<h1 style = 'text-align:center; color: Black;font-size:40px;'>EXECUTION SUMMARY REPORT</h1>", unsafe_allow_html = True)
    st.markdown('<hr style = "border-top: 3px solid orange;">',unsafe_allow_html = True)
    st.write('')
    ...
    for index,row in df_dbq.iterrows():
        ...
        #Reading variables from Parameter file
        path = row["File_Location"]
        S_Query=row["Source_Query"]
        S_PrimaryKey=row["PrimaryKey"]
        T_Query=row["Target_Query"]

        
        #Creation of dataframes
        df1 = pd.read_sql(S_Query, con=Connection1)
        df2 = pd.read_sql(T_Query, con=Connection2)
        df1_rows=df1.shape[0]
        df2_rows=df2.shape[0]
        counts_match = (df1_rows==df2_rows)

        Key_List(df1,df2)
        letter_list = S_PrimaryKey.split(",")
        newList=[]
        for x in letter_list:
            newList.append(x.lower())
        ...
        results.append({'TestName': TN,'Table_Name':TableName,'Source_Count':df1_rows,'Target_Count':df2_rows,'Count_Check':CN,'Comparison_Result':Comparison_Result})
        placeholder.empty()

    return results

def COMPARE_SFDB_DB(connection1,connection2,df_dbq):
    st.markdown("<h1 style = 'text-align:center; color: Black;font-size:40px;'>EXECUTION SUMMARY REPORT</h1>", unsafe_allow_html = True)
    st.markdown('<hr style = "border-top: 3px solid orange;">',unsafe_allow_html = True)
    st.write('')
    ...
    for index,row in df_dbq.iterrows():
        ...
        #Reading variables from Parameter file
        path = row["File_Location"]
        SnowflakeQuerySRC=row["Source_Query"]
        S_PrimaryKey=row["PrimaryKey"]
        SnowflakeQueryTGT=row["Target_Query"]

        
        #Creation of dataframes
        df1 = pd.read_sql_query(SnowflakeQuerySRC, con=connection1)
        df2 = pd.read_sql_query(SnowflakeQueryTGT, con=connection2)
        df1_rows=df1.shape[0]
        df2_rows=df2.shape[0]
        counts_match = (df1_rows==df2_rows)
